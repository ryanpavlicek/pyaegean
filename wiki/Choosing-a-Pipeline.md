...
| --- | --- | --- | --- |
| **Clean digital text** (a well-edited literary passage or `load_work` corpus) | the neural pipeline for accuracy, or the treebank lexicon for attested-gold lemmas without heavy deps | `greek.use_neural_pipeline()` / `greek.use_treebank()` | the neural pipeline is the most accurate (see [Benchmarks](Benchmarks)) but needs the `[neural]` extra and a model fetch; the treebank stays light but only covers attested forms |
