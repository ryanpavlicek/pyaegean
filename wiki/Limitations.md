...
| `greek.load_work` reads top-level textparts only and drops `<note>`/`<bibl>` silently (edition discovery is already cached/authenticated — the WP4 half) | **WP13** (Phase II): nested textpart addressing + carrying notes/bibliography |
| Scansion covers dactylic hexameter + elegiac pentameter only, and **synizesis is declined, never inferred** (a line that needs it raises rather than guesses) | **WP12** (Phase II): iambic trimeter plus a curated synizesis lexicon on the same pattern as the syllabification exceptions (lyric metres only if trimeter lands with room) |
| The offline rule morphology misses irregular, third-declension, and contract paradigms, and doesn't restore accents on reconstructed lemmas (the treebank and neural tiers cover these) | **WP15** (Phase II): Morpheus-backed tables for the offline tier, gated on a license audit |
| The neural pipeline's model is a 518 MB fp32 download (int8 quantization failed its accuracy gate and was rejected) | **WP14** (Phase II): selective quantization under the same ≤0.3-point gate, and optional GPU execution providers — timeboxed, with honest closure if the gate fails again |
