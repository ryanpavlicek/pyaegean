...
The verifier does not use the network, download or execute the neural model, write
bytecode, or create a pyaegean cache. The offline fixture is a regression test, not
the published neural benchmark. Reproducing the neural measurements requires the
...
A pass establishes that the bounded public records have their reviewed bytes and
that the small deterministic result matches. It does not establish that an
interpretation is correct, rerun every benchmark, or amount to external peer review.
Use `--json` for the complete receipt. `--allow-dirty` is for diagnosis only.
...
modification. A mismatch is useful evidence and should be recorded exactly.

People interested in maintenance can choose a bounded ownership slice in
