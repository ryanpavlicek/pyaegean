...
> **Latest PyPI release: v0.57.2 (beta).** Usable and tested, but the API may still shift
...
**Linear B** (Mycenaean Greek, deciphered), the **Cypriot syllabary** (chiefly Arcado-Cypriot
Greek, deciphered; the corpus also carries Eteocypriot and undetermined material), and two scripts
we still *cannot read*: **Linear A** (Minoan) and **Cypro-Minoan**.
...
| | |
|---|---|
| **All four Aegean scripts, one API** | `aegean.load("lineara")` gives the bundled **1,721-inscription** Linear A corpus over the full Unicode Linear A sign repertoire (50 signs carry conventional sound values, the rest are undeciphered); Linear B and Cypro-Minoan add Unicode-built inventories with small *illustrative* text samples (the full DAMOS Linear B corpus is available on demand); the **Cypriot syllabary** bundles a **178-inscription corpus** (*Inscriptiones Graecae* XV 1, BBAW, CC BY 4.0) plus two illustrative samples, for **180 bundled documents** total. The two *deciphered* syllabaries transliterate and bridge into Greek: `po-me → ποιμήν` (Linear B), `pa-si-le-u-se → βασιλεύς` (Cypriot). |
| **A deep Greek NLP pipeline** | Beta Code ↔ Unicode (Beta Code is the plain-ASCII way of typing polytonic Greek), tokenize with named source-preserving sentence policies, syllabify, accent & prosody, **metrical scansion** (scans the *Odyssey*'s opening; rejects lines that require synizesis), reconstructed IPA (Attic / Koine), POS, morphology, and lemmatization. Opt-in backends add attested lemmas/POS (Perseus treebank), a **dictionary registry** (LSJ, Middle Liddell, Cunliffe, Autenrieth, Abbott-Smith) with Logeion deep-links, pure-Python generalizing taggers/lemmatizers, **inflection synthesis** (the inverse lemmatizer), **terminology-rarity** scoring, and **dialect/register** tags from LSJ. |
| **Measured neural NLP** | The opt-in **neural pipeline** (`greek.use_neural_pipeline()`; runs without PyTorch): one jointly-trained model for tagging, full morphology, **dependency parsing** (Universal Dependencies trees), and lemmatization; in plain terms, it reads a Greek sentence and tells you each word's part of speech, grammatical form, dictionary headword, and place in the sentence's structure. Its stable `default` runtime label is the current `grc-joint-v3` artifact; `fast`, `compact`, and `balanced` are visible but unavailable until a successor earns each label from recorded size, latency, and memory evidence. Measured end-to-end through this package at **97.0 UPOS / 96.0 UFeats / 94.3 lemma / 90.2 UAS / 85.7 LAS** on the UD Ancient Greek (Perseus) test benchmark ([protocol, comparison rows, confidence intervals, and limitations](https://github.com/ryanpavlicek/pyaegean/blob/main/docs/benchmarks.md)). |
...
segmented = greek.segment_text("[λόγος.] καί.", policy="papyrus")
segmented.boundaries[0].to_dict(segmented.source)
# exact source span + policy/provenance metadata (use sentence_policy= on pipeline())
...
The core Greek and bundled-corpus examples run offline without heavy dependencies. The CLI, TUI,
DataFrame, neural, and AI examples require their documented extras; network providers also require
network access. Large assets are fetched to a local cache only when you opt in (and never bundled
inside the wheel): the full Linear B corpus
...
- training and independently evaluating a separately versioned successor to the current Greek
  model only after those foundations are frozen;
- producing calibrated runtime variants only after a candidate model passes the cross-domain
  and operational gates; and
- adding license-clean dictionaries, conda-forge packaging, and verified gazetteer coverage.
