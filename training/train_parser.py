...
def _cle(scores: np.ndarray) -> np.ndarray:
    """Max spanning arborescence rooted at node 0. scores[d, h] = score of h→d arc;
    row 0 (the root as dependent) is ignored. Returns head[] with head[0] = -1."""
    n = scores.shape[0]
    ...
    # contract the cycle into one node; renumber the rest
    keep = [v for v in range(n) if v not in cyc]
    new_of = {v: i for i, v in enumerate(keep)}
    c_new = len(keep)
    m = len(keep) + 1
    new_scores = np.full((m, m), -np.inf)
    best_in = {}    # for arcs entering the cycle: (outside head) -> (cycle dep)
    best_out = {}   # for arcs leaving the cycle: (outside dep) -> (cycle head)
    for d in range(1, n):
        for h in range(n):
            if d == h:
                continue
...
def decode_mst(arc_scores: np.ndarray) -> list[int]:
    """Heads for one sentence from arc scores [W, W+1] (col 0 = ROOT), single-root.
    Returns 0-based heads in the {0=root, 1..W} convention (i.e. CoNLL-U HEAD values)."""
    w = arc_scores.shape[0]
    # node 0 = root, nodes 1..W = words; scores[d, h]
    full = np.full((w + 1, w + 1), -np.inf)
    full[1:, :] = arc_scores
    heads = _cle(full)
    root_children = [d for d in range(1, w + 1) if heads[d] == 0]
    if len(root_children) > 1:                   # enforce a single root
        best = max(root_children, key=lambda d: full[d, 0])
        constrained = full.copy()
        for d in root_children:
            if d != best:
                constrained[d, 0] = -np.inf
        heads = _cle(constrained)
