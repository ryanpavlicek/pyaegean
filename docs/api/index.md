...
- [`aegean.greek`](greek.md): the Greek NLP pipeline (normalize, tokenize, named sentence policies, scan, tag, lemmatize, parse), `segment_text()` / `segment_sentences()` with validated plugin results and exact boundary spans, `pipeline_tokens()` for typed editorial forms, isolated `GreekPipeline` instances with serializable configuration, explicit long-input policies, exact `AnalysisReceipt` provenance, manifest-validated neural bundles, lossless CoNLL-U document I/O, plus work discovery: `catalog()` (the full ~1,800-work index), `popular_works()`, and `nt_books()`.
