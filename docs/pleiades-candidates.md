...
*ToposText cross-ids* are a possible future addition (the roadmap notes them),
but ToposText has no clean id API, so they are out of scope here.

## Method

`scripts/`-style probe: for each unaligned site, search Pleiades
(`/search_rss?portal_type=Place`), fetch each candidate's `/places/<id>/json`,
and accept only a place whose `reprPoint` is within ~5 km of ours **and** whose
title/description matches the site. The probe is not committed (a one-off
research tool); this page records its verified result.
