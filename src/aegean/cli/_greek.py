...
def scan(
    line: str = TEXT_ARG,
    meter: str = typer.Option("hexameter", "--meter", help="hexameter or pentameter."),
    json_out: bool = JSON_OPT,
) -> None:
    """Metrical scansion (dactylic hexameter / elegiac pentameter).

    Synizesis is declined, never inferred: a line that only fits via synizesis
    exits 1 with the reason rather than forcing a fit."""
    from aegean import greek
