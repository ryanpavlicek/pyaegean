...
**Opt-in.** Call :func:`use_lsj` to download the Perseus LSJ (TEI) into the user
cache, build a *lemma → entry* index (also cached), and then :func:`gloss` /
:func:`lookup` turn a Greek word into its dictionary entry. Looking up an inflected
form works: it tries the form, then (on a miss) lemmatizes — using the treebank
backend if active — and retries. Default behaviour without :func:`use_lsj` touches
no network.
...
here with :func:`aegean.greek.betacode_to_unicode`.
"""
...
class LexiconNotLoadedError(RuntimeError):
    """Raised when gloss/lookup is called before :func:`use_lsj`."""
...
def use_lsj(*, build: bool = True, force: bool = False) -> LSJLexicon:
    ...
    ``force=True`` to rebuild. Then :func:`gloss` / :func:`lookup` resolve words
    against it.
    """
    global _ACTIVE
...
def gloss(word: str) -> str | None:
    """Concise LSJ gloss for a word; requires :func:`use_lsj`. ``None`` if unknown."""
    if _ACTIVE is None:
...
def lookup(word: str) -> LSJEntry | None:
    """Full LSJ entry for a word; requires :func:`use_lsj`. ``None`` if unknown."""
    if _ACTIVE is None:
        raise LexiconNotLoadedError("LSJ is not loaded — call aegean.greek.use_lsj() first")
