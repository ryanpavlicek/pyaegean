...
def _blocks(part: Any) -> list[str]:
    """The physical lines of a textpart: verse ``<l>`` lines, else ``<p>`` blocks,
    else the part's own flattened text."""
    lines = [_line_text(line) for line in part.iter(f"{_TEI}l")]
    lines = [ln for ln in lines if ln]
    if lines:
        return lines
    paras = [_line_text(p) for p in part.iter(f"{_TEI}p")]
...
def parse_tei_work(blob: bytes, work: str) -> tuple[str, str, list[Document]]:
    """Parse one TEI work file into ``(title, author, documents)``.

    One `Document` per top-level textpart of the edition div (an Iliad book, a
    prose chapter run); verse ``<l>`` lines or ``<p>`` blocks become the physical
    lines; ``<note>``/``<bibl>`` editorial subtrees are excluded from the text."""
    ...
    for i, part in enumerate(parts, start=1):
        ...
        tokens, lines = _tokens_for(_blocks(part))
        if not tokens:
            continue
        docs.append(
            Document(
                id=f"{work}:{n}", script_id="greek", tokens=tokens, lines=lines,
                meta=DocumentMeta(name=f"{title} — {subtype} {n}".strip()),
            )
        )
...
def load_work(
    ...
    *,
    ...
    file is fetched once into the cache (network on first use only) and parsed
    into one `Document` per top-level textpart (book/chapter), with verse lines
    or paragraphs as the physical lines. Raises
    `aegean.data.DataNotAvailableError` when the work can't be found/fetched."""
    from ...core.corpus import Corpus

    blob, src, filename = _fetch_xml(work, source, edition, force)
    title, author, docs = parse_tei_work(blob, work)
    ...
    provenance = Provenance(
        ...
        url=f"https://github.com/{repo}/blob/{ref}/{_work_dir(work)}/{filename}",
        data_version=f"{repo}@{ref[:12]}",
        notes=(f"fetched to cache; one Document per textpart of {work}",),
    )
